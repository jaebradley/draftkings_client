...
from draft_kings.model.draft_group import League, Game, Contest, StartTime
...
class TestDraftGroup11513MockedHTTPResponse(TestCase):
    def setUp(self) -> None:
        with open(
            os.path.join(ROOT_DIRECTORY, "tests/files/draft_group_details/11513.json"), encoding="utf-8"
        ) as data_file:
    ...
    def test_league_details(self):
        self.assertListEqual(
            [League.model_construct(league_id=4, name="National Basketball Association", abbreviation="NBA")],
            self.result.leagues,
        )
    ...
    def test_contest_details_values(self):
        self.assertEqual(
            Contest.model_construct(game_type_description="SalaryCap", type_id=5), self.result.contest_details
        )

    def test_start_time_details(self):
        self.assertEqual(
            StartTime.model_construct(
    ...
    def test_state(self):
        self.assertEqual("Historical", self.result.draft_group_state)
